...
-- 문자열 연결
select concat('마당', concat('마당','서점')) from dual;
...
-- 고객 중에서 같은 성을 가진 사람이 몇 명이나 되는지 성별 인원 수
select substr(name, 1, 1) 성, count(name) 인원_수 from customer
...
-- 마당서점이 7월7일에서 7월9일까지 판매한 책 이름과 가격
select o.orderdate, bookname, saleprice from orders o
...
-- Null 값에 대한 연산
select price+100 from mybook;    --null에 더해도 null이다.
...
select rownum, a.* from
    (select * from book order by price desc) a
    where rownum >= 3 and rownum < 6 ;      -- rownum 비교는 LT, LE만 가능
